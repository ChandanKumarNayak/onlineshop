...
if(isset($_GET['type']) && $_GET['type'] != ''){
    ...
    if($type == 'download_Prod_cat_pdf'){

        $sql = "SELECT * FROM product_cat ORDER BY cat_id DESC";
  ...
  <table id="pdf-tbl">
            <thead>
                ...
                while($row=mysqli_fetch_assoc($sql_query)){
                    $html.='<tbody><tr><td>'.$i.'</td><td><img src="'. PROD_CAT_IMAGE_PATH . get_safe_value($row['cat_img']).'" class="tbl-inside-img" /></td><td>'.get_safe_value($row['cat_name']).'</td><td>'.get_safe_value($row['cat_status']).'</td></tr></tbody>';
                    $i++;
                }
